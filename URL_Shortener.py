...
class url_shortner:
        ...
        def create(self):
                if self.url.get() == "":
                        messagebox.showerror("Error","Silahkan Paste URL")
                else:
        ...
        def __init__(self):
                ...
                Label(self.root,text="Paste Url Disini ..",font=('verdana',10,'bold'),fg="purple").place(x=50,y=50)

                self.url = Entry(self.root,width=50,bg="lightgrey",relief=GROOVE,borderwidth=2,border=2)
                self.url.place(x=50,y=80)

                self.button = Button(self.root,relief=GROOVE,text="Buat",font=('verdana',8,'bold'),bg="purple",fg="white",command=self.create)
